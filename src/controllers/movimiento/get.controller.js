...
const listarPorCaja = async (req, res) => {
  try {
    ...
    const { code, message, movimientos } =
      await movimientoService.listarPorCaja(caja_id);
...
const listarPorClave = async (req, res) => {
  try {
    ...
    const { code, movimientos } = await movimientoService.listarPorClave(
      clave,
      valor,
    );
...
const listarPorRango = async (req, res) => {
  try {
    ...
    const { code, movimientos } = await movimientoService.listarPorRango(
      fechaInicio,
      fechaFin,
    );
    res.status(code).json({ movimientos });
